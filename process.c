...
void print_process_info(ProcPtr proc_list, int num_of_proc){
    ...
    for(i=0;i<num_of_proc;i++){
        printf("pid : %-3d| arrival : %-4d| cpu_burst : %-3d| io_burst : %-3d| priority : %-3d\n", proc_list[i].pid,
                proc_list[i].arrival, proc_list[i].cpu_burst, proc_list[i].io_burst, proc_list[i].priority);
    }
    puts("");
}
